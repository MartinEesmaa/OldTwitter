...
if (
    /^\/[A-z-0-9-_]{1,15}\/status\/\d{5,32}\/(photo|video)\/\d+$/.test(realPath)
) {
...
const TRANSLATORS = {
    ...
    ko: [["Nyankodasu", "https://github.com/Nyankodasu"], ["HY"]],
    pl: [["lele"], ["nomi", "/nomisigns"]],
...
let page =
    realPath === ""
        ...
        : pages.find(
              (p) =>
                  (!p.exclude || !p.exclude.includes(realPath)) &&
                  (p.paths.includes(realPath) ||
                      p.paths.find(
                          (r) => r instanceof RegExp && r.test(realPath)
                      ))
          );
(async () => {
    if (!page) return;
    ...
    document
        ...
        .style.setProperty(
            ...
            vars.transitionProfileBanner ? ".1s" : "0s"
        );
    if (vars.systemDarkMode) {
        var matchMediaDark = window.matchMedia("(prefers-color-scheme: dark)");
        var matchMediaLight = window.matchMedia(
    ...
    let [LOC_DATA, LOC_EN_DATA, html, css, header_html, header_css] =
        await Promise.all([
            fetch(
                chrome.runtime.getURL(`_locales/${LANGUAGE}/messages.json`)
            ).then((response) => response.json()),
            fetch(chrome.runtime.getURL(`_locales/en/messages.json`)).then(
                (response) => response.json()
            ),
            fetch(
                chrome.runtime.getURL(`layouts/${page.name}/index.html`)
            ).then((response) => response.text()),
            fetch(chrome.runtime.getURL(`layouts/${page.name}/style.css`)).then(
                (response) => response.text()
            ),
            fetch(chrome.runtime.getURL(`layouts/header/index.html`)).then(
                (response) => response.text()
            ),
            fetch(chrome.runtime.getURL(`layouts/header/style.css`)).then(
                (response) => response.text()
            ),
        ]);
    LOC = LOC_DATA;
    ...
    if (msgs) {
        for (let i = 0; i < msgs.length; i++) {
            let m = msgs[i].slice(6, -2);
            if (LOC[m])
                header_html = replaceAll(header_html, msgs[i], LOC[m].message);
        }
    }
    ...
    fetch(chrome.runtime.getURL("libraries/gif.worker.js"))
        .then((response) => response.text())
        .then((text) => {
            window.gifWorkerUrl = window.URL.createObjectURL(
